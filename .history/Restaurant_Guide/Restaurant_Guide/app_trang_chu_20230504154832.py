...
def huongdan_app(key_app):
    connection_2014 = connect_SQL_Web_NH_172_16_8_19()
    conn_2014 = connection_2014.cursor()
    sql = "SELECT id,TACVU_CHINH,TACVU_PHU,FILE_DINHKEM,TENKHONGDAU_LON,TENKHONGDAU_NHO,VERSION_APP,NOTE,TEN_FILE,ACTIVE FROM dbo.HUONGDAN_CHUNG_APP WHERE ID_APP_ALL='"+str(key_app)+"' ORDER BY NGAY_INSERT"
    sql1 = str(sql)
    ...
    if len(danh_sach) != 0:
        ...
        for item in danh_sach:
            ...
            danhsach_html += '''
                        <tr>
                            <td class="chinhlai_input_the_td"><input name="TACVU_CHINH" id="'''+str(id)+'''" class="input_table" value="'''+str(tacvu_chinh)+'''" onblur="iWillCallWhenBlur(this)"></td>
                            <td class="chinhlai_input_the_td"><input name="TACVU_PHU" id="'''+str(id)+'''"  class="input_table" value="'''+str(tacvu_phu)+'''" onblur="iWillCallWhenBlur(this)"></td>
                            <td class="chinhlai_input_the_td"><a href="link/to/your/download/file" download>'''+str(ten_file)+'''</a></td>
                            <td class="chinhlai_input_the_td"><input name="VERSION_APP" id="'''+str(id)+'''"  class="input_table" value="'''+str(version_app)+'''" onblur="iWillCallWhenBlur(this)"></td>
                           
                            <td class="chinhlai_input_the_td"><select style="width: 100%; height: 45px; border: 0; padding-left: 15px;" name="ACTIVE" id="'''+str(id)+'''" onblur="iWillCallWhenBlur(this)">
                                <option label="Tắt Kích Hoạt" '''+str(trangthai2)+'''>0</option>
                                <option label="Kích Hoạt"  '''+str(trangthai1)+'''>1</option>
                            </select></td>
...
# =========================== Update APP Chi tiet Start=====================================
# =========================== Update APP Chi tiet Start=====================================
@app.route('/huongdan_app_chitiet/<id_app>', methods=['GET', 'POST'])
def huongdan_app_chitiet(id_app):
    import unidecode
    connection_2014 = connect_SQL_Web_NH_172_16_8_19()
    conn_2014 = connection_2014.cursor()
    ...
    if cotcanupdate =='TACVU_CHINH':
        sql ="UPDATE dbo.HUONGDAN_CHUNG_APP SET "+str(cotcanupdate)+" = N'"+str(giatri_update)+"',TENKHONGDAU_LON = N'"+str(giatri_update_khongdau)+"' WHERE id='"+str(id_update)+"'"
        sql1 = str(sql)
        conn_2014.execute(sql1,)
        conn_2014.commit() 
        # print('OK TACVU_CHINH') 
        return 'ok'

    if cotcanupdate =='TACVU_PHU':
        sql ="UPDATE dbo.HUONGDAN_CHUNG_APP SET "+str(cotcanupdate)+" = N'"+str(giatri_update)+"',TENKHONGDAU_NHO = N'"+str(giatri_update_khongdau)+"' WHERE id='"+str(id_update)+"'"
        sql1 = str(sql)
        conn_2014.execute(sql1,)
        conn_2014.commit() 
        # print('OK TACVU_PHU')
        ...
        print(cotcanupdate)
        print(giatri_update)
        sql ="UPDATE dbo.HUONGDAN_CHUNG_APP SET "+str(cotcanupdate)+" = N'"+str(giatri_update)+"' WHERE id='"+str(id_update)+"'"
        sql1 = str(sql)
        conn_2014.execute(sql1,)
        conn_2014.commit() 
        # print('OK Khác') 
    ...
    # return render_template('login.html',USER = user1 , PASS = pass1)
# =========================== Update APP Chi tiet App END=====================================
# =========== Update APP Chi tiet END===================
