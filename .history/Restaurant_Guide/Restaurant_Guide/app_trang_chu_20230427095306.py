...
def huongdan_app(key_app):
    connection_2014 = connect_SQL_Web_NH_172_16_8_19()
    conn_2014 = connection_2014.cursor()
    sql = "SELECT id,TACVU_CHINH,TACVU_PHU,FILE_DINHKEM,TENKHONGDAU_LON,TENKHONGDAU_NHO,VERSION_APP,NOTE FROM dbo.HUONGDAN_CHUNG_APP WHERE ID_APP_ALL='"+str()+"' AND ACTIVE='1' ORDER BY NGAY_INSERT"
    conn_2014.execute(sql,)
    danh_sach = conn_2014.fetchall()              
    if danh_sach is not None:
        for item in danh_sach:
